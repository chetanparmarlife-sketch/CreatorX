...
public class DeliverableService {
    ...
    public Page<DeliverableDTO> getDeliverables(String creatorId, SubmissionStatus status, int page, int size) {
        Pageable pageable = org.springframework.data.domain.PageRequest.of(page, size);
        Page<DeliverableSubmission> submissionsPage;

        if (status != null) {
            submissionsPage = deliverableRepository.findByCreatorIdAndStatus(
                    creatorId, status, pageable);
        } else {
            submissionsPage = deliverableRepository.findByCreatorId(
                    creatorId, pageable);
        }
    ...
    /**
     * Get deliverables for brand (all campaigns owned by brand, paginated)
     */
    @Transactional(readOnly = true)
    public Page<DeliverableDTO> getDeliverablesForBrand(String brandId, SubmissionStatus status, int page, int size) {
        Pageable pageable = org.springframework.data.domain.PageRequest.of(page, size);
        Page<DeliverableSubmission> submissionsPage;
    ...
    /**
     * Get deliverables for a specific campaign (brand only, paginated)
     */
    @Transactional(readOnly = true)
    public Page<DeliverableDTO> getDeliverablesByCampaign(String campaignId, String brandId, SubmissionStatus status,
            int page, int size) {
     ...
     */
    @Transactional(readOnly = true)
    public Page<DeliverableDTO> getDeliverablesForAdmin(String brandId, String campaignId, SubmissionStatus status,
            Pageable pageable) {
    ...
    /**
     * Get deliverables for a specific application (brand only, paginated)
     */
    @Transactional(readOnly = true)
    public Page<DeliverableDTO> getDeliverablesByApplication(String applicationId, String brandId,
            SubmissionStatus status, int page, int size) {
        // Verify brand owns the campaign
    ...
    public List<DeliverableHistoryDTO> getDeliverableHistory(String submissionId) {
        ...
        // Since we update existing submissions, we'll create history from the current
        // submission
    ...
    private void sendNotification(String userId, NotificationType type, String title, String body,
