...
export default function HowItWorks() {
  ...
  return (
    <section id="how-it-works" className="py-24 bg-gradient-to-br from-[#0f4c3a] via-[#134e3a] to-[#0d3d2e] relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <div className="text-center mb-16">
          ...
          <div className="inline-flex bg-white/10 backdrop-blur-sm rounded-full p-1.5">
            <button 
              ...
              className={`px-6 py-3 rounded-full font-medium transition-all duration-300 ${campaignType === "shipping" ? "bg-white text-black shadow-lg" : "text-white hover:bg-white/10"}`}
            >
              Shipping Campaign
            </button>
            <button 
              onClick={() => setCampaignType("non-shipping")}
              className={`px-6 py-3 rounded-full font-medium transition-all duration-300 ${campaignType === "non-shipping" ? "bg-white text-black shadow-lg" : "text-white hover:bg-white/10"}`}
            >
        ...
        <div className="grid lg:grid-cols-2 gap-12 items-start">
            ...
            {steps.map((step, index) => (
              ...
              >
                <div className="flex items-start gap-4">
                  <div className={`text-3xl transition-transform duration-300 ${activeStep === index ? 'scale-110' : ''}`}>
                    {step.icon}
                  </div>
                  <div className="flex-1">
                    <h3 className={`text-lg font-bold mb-2 transition-colors duration-300 ${activeStep === index ? 'text-[#c8ff00]' : 'text-white'}`}>
                      {step.title}
                    </h3>
          ...
          <div className="bg-white rounded-3xl p-8 shadow-2xl transform hover:scale-[1.02] transition-transform duration-500">
            ...
            <div className="flex gap-6 mb-8 border-b border-gray-200 pb-4">
              <span className="text-sm font-semibold text-violet-600 border-b-2 border-violet-600 pb-3">1. About brand</span>
            ...
            <div className="space-y-6">
              <div>
                <h4 className="text-xl font-bold text-gray-900 mb-2">About brand</h4>
                <p className="text-gray-500 mb-4">Tell us the brand name and tagline</p>
                <input 
                  type="text" 
                  placeholder="Enter brand name" 
                  className="w-full px-5 py-4 border-2 border-gray-200 rounded-xl text-gray-900 placeholder-gray-400 focus:border-violet-500 focus:outline-none transition-colors"
                />
              </div>
              <div>
                <p className="text-gray-500 mb-4">Category of your product (select one)</p>
                <div className="flex flex-wrap gap-3">
                  <span className="px-5 py-2.5 bg-violet-100 text-violet-700 rounded-full text-sm font-medium cursor-pointer hover:bg-violet-200 transition-colors">Fashion wear</span>
                  <span className="px-5 py-2.5 bg-gray-100 text-gray-600 rounded-full text-sm font-medium cursor-pointer hover:bg-gray-200 transition-colors">Innerwear</span>
                  <span className="px-5 py-2.5 bg-gray-100 text-gray-600 rounded-full text-sm font-medium cursor-pointer hover:bg-gray-200 transition-colors">Beauty</span>
                </div>
              </div>
            </div>
          </div>
