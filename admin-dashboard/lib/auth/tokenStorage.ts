/**
 * tokenStorage.ts
 * Manages admin auth token storage using HttpOnly cookies via API routes.
...
export const tokenStorage = {
  ...
  async getAccessToken(): Promise<string | null> {
    // Read the admin access token through a server-side cookie route instead of localStorage.
  ...
  async clearTokens() {
    // Clear admin HttpOnly cookies via API route instead of removing readable localStorage token keys.
  ...
  needsRefresh(): boolean {
    // The old localStorage expiry timestamp is gone; failed API requests handle auth expiry.
    return false
  },
...
export async function isAuthenticated(): Promise<boolean> {
  // Check admin authentication through the HttpOnly cookie-backed token route.
  return tokenStorage.isAccessTokenValid()
}

export async function getAccessToken(): Promise<string | null> {
  // Return the admin access token via API route instead of reading localStorage directly.
...
export async function clearAuth(): Promise<void> {
  // Clear admin auth cookies through the server route instead of localStorage.removeItem.
